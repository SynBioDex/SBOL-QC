...
def write_label(ax, label_text, x_pos, opts=None):
    ...
    if opts != None:
        ...
        if 'label_weight' in opts.keys():
            label_weight = opts['label_weight']
        if 'horizontal_alignment' in opts.keys():
            horizontal_alignment = opts['horizontal_alignment']
    ax.text(x_pos + label_x_offset, label_y_offset, label_text, horizontalalignment=horizontal_alignment,
            verticalalignment='center', fontsize=label_size, fontweight=label_weight, fontstyle=label_style,
...
def sbol_terminator(ax, type, num, start, end, prev_end, scale, linewidth, opts):
    ...
    l1 = Line2D([start + dir_fac * (x_extent / 2.0), start + dir_fac * (x_extent / 2.0)], [0, dir_fac * y_extent],
                linewidth=linewidth,
...
def stick_figure(ax, type, num, start, end, prev_end, scale, linewidth, opts):
    ...
    if start > end:
        ...
        dash2 = Line2D([end + ((start - end) / 2.0), end + ((start - end) / 2.0)],
                       [-y_extent / 2, -y_extent + (x_extent / 2.0)],
                       linewidth=linewidth, color=color, zorder=8, linestyle=linestyle)
...
def trace_rbs(ax, type, num, start_bp, end_bp, prev_end, scale, linewidth, opts):
    ...
    ax.add_line(l1)
    c1 = Ellipse((start_bp, dir_fac * y_extent), width=(x_extent * scale), height=y_extent * 0.4, color=color,
                 zorder=14)
    ax.add_artist(c1)
    # Shade the promoter area (normally smaller than symbol extent)
    p2 = Polygon([(start_bp, -highlight_y_extent),
...
def trace_terminator(ax, type, num, start_bp, end_bp, prev_end, scale, linewidth, opts):
    ...
    l2 = Line2D([start_bp - (x_extent * scale), start_bp + (x_extent * scale)],
                [dir_fac * y_extent, dir_fac * y_extent], linewidth=linewidth, color=color, zorder=14)
    ax.add_line(l1)
    ax.add_line(l2)
...
class DNARenderer:
    ...
    def renderDNA(self, ax, parts, part_renderers, regs=None, reg_renderers=None):
        ...
        for part in parts:
            ...
            # label part with name
            #name = part['name']
            #center = (part['start'] + part['end']) / 2
            #ax.text(center, -5, name, bbox=dict(edgecolor='black', facecolor='red', alpha=0.5), horizontalalignment = 'center', fontsize = 12)
...
def plot_sbol_designs(axes, dna_designs, regulations=None, plot_params={}, plot_names=None):
    ...
    return max_dna_len, [(-0.01 * max_dna_len) - left_pad, max_dna_len + (0.01 * max_dna_len) + right_pad], [
        -plot_params['axis_y'], plot_params['axis_y']]
...
def save_sbol_designs(filename, dna_designs, regulations=None, plot_params={}, plot_names=None):
    ...
    max_dna_len, lims, params = plot_sbol_designs(axes, dna_designs, regulations=regulations, plot_params=plot_params,
